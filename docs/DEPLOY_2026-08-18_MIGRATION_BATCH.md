...
# 3 — الهجرة في حاوية تُرمى.  ← نقطة الأمان الكاملة
#     أي فشل هنا رسالة خطأ فقط: لا set -e، لا restart، لا حلقة إقلاع،
#     والموقع ما زال على النسخة القديمة.
...
أي `Traceback` ⇒ **توقّف.** لم يتغيّر شيء بعد؛ الموقع يعمل.

```bash
# 4 — نقطة اللاعودة. لا تصلها إلا والمخرَج نظيف.
