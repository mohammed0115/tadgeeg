...
والسبب المُعلَن ليس الرقم 115، بل السطر نفسه:

```python
is_blocking = _infer_blocking(category, normalized_severity)
```

**الحلّ لا يقرأ حكمًا مسجَّلًا — يستنتجه.** فقاعدة تصير حاجبة لأن خطورتها `high`،
لا لأن مدقّقًا قرّر ذلك. وهذا نمط `VAT-001` بعينه: قيمة مُستنتَجة تُعرَض كتحديد —
**والحجب أخطر من نسبة ضريبة.**

**والرقم 236 يحسمه بمعزل عن الحجب:** `SI-001` ⇒ `CMT-###` يفصل كل مرجع مخزَّن،
فأي `AuditResult` قديم يحمل `SI-001` يصير بلا مقابل.

**ولا يُحذف:** الحزمة تحمل البيانات، والمحجوب يحمل آلة الاستعلام، وسبعة مواضع
تستدعيها.

**ما نُفِّذ بموجب القرار:**

| | |
|---|---|
| رأس `apps/rule_engine/catalog.py` | يوثّق القرار وأرقامه — تعليق، صفر سطر شيفرة |
| `tests/test_catalog_stays_shadowed.py` | يفشل يوم تُصدّر الحزمة الحلّ ⇒ الوصل قرار مرئي لا انزلاق |
| المواضع السبعة | `ImportError` ⇒ تحذير **مرّة واحدة** · وأي استثناء آخر ⇒ `error` باسم القاعدة والسبب · والسلوك الاحتياطي كما هو · **ولا رفع** |

---

## ١٠. بنود مسجَّلة، غير مفتوحة

| البند | الحالة |
|---|---|
| الرقم ١٢ على الإنتاج (مستندات مُعتمَدة كانت ستُحجَب) | **سؤال عن مستقبل لن يقع** بعد قرار عدم الوصل. مسجَّل |
| `apps/authentication/tests.py` يحجب `tests/test_mfa_token_issuance.py` | اختبار MFA لم يُشغَّل قطّ — **شحنة تالية** |
| 402 مقابل 403 | أي طبقة ترفض أوّلًا؟ رسالة «اختر باقة» تُخفي سبب الرفض الحقيقي |
| الباقات المحاسبية الثلاث | استعلام الإنتاج عند المشغّل |
| `9c02c2c` وُصف «وثائق فقط» وحمل نقل ملف | مدفوع؛ لا يُعاد كتابة تاريخه. مُصرَّح به في `a20bf7c` |
