# هذا الملف محجوب عمدًا بحزمة `catalog/` التي تحمل الاسم نفسه، ويبقى كذلك.
#
# ولا يُوصَل. قِيس أثر وصله على 236 قاعدة حيّة — docs/CATALOG_SHADOW_IMPACT.md:
#
#   · `resolve_rule_catalog_metadata` لا ترفع استثناءً عند عدم التطابق، بل
#     تُصنّع مدخلًا وتُسجّله، و`is_blocking` فيه **مستنتَج** من الفئة والخطورة
#     لا مقروء من حكم مسجَّل. فقاعدة تصير حاجبة لأن خطورتها `high`، لا لأن
#     مدقّقًا قرّر ذلك. و115 قاعدة تبدأ بمنع الاعتماد، كلّها `high`.
#
#   · و`_build_generated_catalog_code` يغيّر **236 رمزًا من 236**:
#     `SI-001` ⇒ `CMT-###`. فكل مرجع مخزَّن أو مصدَّر بالرمز القديم يفقد
#     مقابله — وهذا وحده يُسقط الوصل، بمعزل عن الحجب.
#
# ولا يُحذف: `RuleCategory` و`RuleCatalogEntry` و`get_rules_for_doc_type` و
# `get_blocking_rules` و`get_rules_by_category` لا مقابل لها في الحزمة، وسبعة
# مواضع في أربعة تطبيقات تستدعي الحلّ. الحزمة تحمل البيانات، وهذا الملف يحمل
# آلة الاستعلام عنها.
#
# وقد عمل عشرين يومًا: أُنشئ في 9a94bd6 (2026-04-12) مع مستدعيه، وأسكته
# `catalog/__init__.py` بصفر بايت في decd8e1 (2026-05-02).
#
# tests/test_catalog_stays_shadowed.py يفشل يوم تُصدّر الحزمة هذا الحلّ —
# ليكون الوصل قرارًا مرئيًّا لا انزلاقًا.
