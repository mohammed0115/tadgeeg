...
## ١١. ملحق التشغيل الحيّ — 2026-08-15

قياس على **قاعدة التشغيل** `db_runtime.sqlite3` بعد رفع حقيقي من المتصفح
(`invoice_Toby Ritter_8265.pdf`)، لا على قاعدة اختبار. **قراءة فقط؛ لم يُعدَّل صف.**

### ١١-أ · عائق سبق القياس: هجرة غير مُطبَّقة

أول رفع فشل بـ`table invoices has no column named audit_document_id`.
السبب: `0017_invoice_audit_document` و`0018_..._backfill` غير مؤشَّرتين، والعمود
غائب (78 عمودًا). و`Invoice.objects.create()` يُدرج كل حقل ملموس، فالعمود
المفقود يكسر أول كتابة في كل رفع.

طُبِّقت الهجرتان بتصريح صريح، بعد نسخة احتياطية من القاعدة:

```
Applying invoices.0017_invoice_audit_document...
  0017 invoice audit-document backfill: total=67 filled=0
  left_null_no_candidate=67 left_null_ambiguous=0  OK
Applying invoices.0018_invoice_audit_document_backfill...  OK
→ audit_document_id present: True   (79 columns)
```

`filled=0` متوقّع لا فاشل: الملء الرجعي محافظ، ولا يربط إلا تطابقًا وحيدًا داخل
المنظمة نفسها، و67 فاتورة سابقة تسبق الجسر.

🔴 **وهذا بند تغطية، لا حادثة بيئة:** المجموعة تبني قاعدة جديدة وتُهاجرها، فهي
لا ترى أبدًا أن قاعدة التشغيل متأخّرة. النتيجة يومها: `4031 passed` ورفعٌ يفشل
في كل مرة من المتصفح.

### ١١-ب · 🔴 FI-01 مُعاد إنتاجه على بيانات حقيقية

صفّان بالرقم `8265` من الملف نفسه:

| | الصف 1 | الصف 2 |
|---|---|---|
| `created_at` | `11:05:58.730174` | `11:06:04.662879` |
| `audit_session_id` | `0a9db46b…` | `458703b0…` |
| `batch_id` | `0b50077b…` | `27ce465a…` |
| `audit_document_id` | `a9bd8301…` | `a324d711…` |
| `file_hash` | `f9d28a5b0f386a3368e8` | `f9d28a5b0f386a3368e8` — **متطابق** |
| `is_duplicate` | 1 | 1 |

**الفارق 5.932705 ثانية · جلستان · دفعتان · هاش واحد.**

هذا يُثبت حكم §4 على بيانات إنتاج: الازدواج **بالطلب** لا بالمسار. ولو كان
المصدر حلقةً واحدة لاشتركا في الجلسة والدفعة — وهما مختلفتان.

وأثر متراكم في القاعدة: **7 بصمات ملفات لكلٍّ أكثر من فاتورة ⇒ 7 صفوف زائدة**
من أصل 69. و24 صفًّا مؤشَّرًا `is_duplicate=1`.

### ١١-ج · لقطة جودة الاستخلاص على القاعدة كلها

| المقياس | العدد | من 69 |
|---|---:|---|
| `ocr_confidence = 0` | **68** | 98.6% |
| `currency = 'SAR'` | **68** | 98.6% |
| `total_amount = 0` | 10 | 14.5% |
| `invoice_date IS NULL` | 6 | 8.7% |
| `is_duplicate = 1` | 24 | 34.8% |

### ١١-د · 🔴 عيب جديد — المستخلص الاحتياطي يكتب قيمًا **واثقة وخاطئة**

المستند PDF بطبقة نصّية سليمة، وفيها حرفيًا `SuperStore` و`Date: Jul 20 2012`
و`Subtotal: $785.87` و`Total: $925.51`. والمحفوظ:

| الحقل | في المستند | المحفوظ |
|---|---|---|
| `vendor_name` | `SuperStore` | 🔴 `Ship Mode: First Class` |
| `invoice_date` | `Jul 20 2012` | `None` |
| `subtotal` | `$785.87` | `0.00` |
| `total_amount` | `$925.51` | `0.00` |
| `ocr_confidence` | — | `0.0` |

الآلية المقاسة: `_extraction_method = 'pdf_text_layer'` مع
`_extraction_error = "Error code: 401 … invalid_api_key"`. فطبقة الذكاء سقطت،
وعمل الاحتياطي، فالتقط سطر `label: value` خاطئًا كاسم مورد.

وهذا هو النمط نفسه الذي أنتج `oice`: القاعدة تحمل أيضًا
`'_name": "Acme Trading",'` و`'_name": "ZipVendor",'` — أي أن **FI-05 ليس محصورًا
في JSON**، بل هو سلوك الاحتياطي عبر الصيغ.

⚠️ **حدّ الإثبات:** مفتاح OpenAI في هذه البيئة غير صالح. فلا أستطيع أن أنفي أن
مفتاحًا صالحًا كان سيستخرج القيم صحيحة. **لكن العيب ليس في سقوط الطبقة —
العيب أن الاحتياطي يكتب قيمة خاطئة بدل أن يترك الحقل فارغًا.** حقل فارغ
يُرى؛ و`Ship Mode: First Class` في خانة المورد يُقرأ كمورد.

### ١١-هـ · 🔴 عيب جديد — العملة تُختم `SAR` بلا دليل

المستند بالدولار (`$925.51`)، والمحفوظ `SAR`.
المصدر: `core/services/normalization.py:106` —
`def normalize(self, payload, *, default_currency: str = "SAR")`.

الافتراضي يعمل حين لا تُستخرج عملة. والنتيجة صفّ مالي يقول
**SAR 925.51** عن مستند يقول **USD 925.51** — وهما مبلغان مختلفان.
68 من 69 صفًّا في القاعدة تحمل `SAR`.

### ١١-و · 🔴 عيب جديد — `VAT-001` تجتاز على قيمة افتراضية لا على دليل

الواجهة تعرض: `VAT-001 · اجتاز · نسبة الضريبة 15.0% ✓` — على فاتورة
`vat_amount = 0.00` بالدولار الأمريكي ولا سطر ضريبة فيها أصلًا.

```
core/services/invoice_validator.py:302-313
    expected_rate = 15.0
    actual_rate   = float(invoice.vat_rate or 0)
    ok = abs(actual_rate - expected_rate) < 0.5
```

و`apps/invoices/models.py:83` — `vat_rate = DecimalField(default=15)`.

⇒ حين لا يُستخرج شيء، تُقارن القاعدة **الافتراضي بالمتوقَّع**، فتجتاز دائمًا.
ولا يوجد شرط «هل استُخرجت النسبة؟». وللمقارنة، `VAT-002` في السطر 316 يحرس
نفسه بـ`if sub > 0` — فالحارس موجود في القاعدة المجاورة وغائب هنا.

🔴 **وهذا هو نمط العيوب المعلن في `CLAUDE.md` حرفيًا: ادّعاء يُقرأ كدليل.**
درجة تدقيق 62% على سجلّ كل حقوله المالية أصفار، وفيه قاعدة ضريبية «مجتازة»
لم تفحص شيئًا. وهي أخطر من الحقل الفارغ، لأن المراجع يرى ✓ فيعبر.

### ١١-ز · ما لم يُقَس هنا

- لم أقس أثر مفتاح OpenAI صالح على ١١-د.
- لم أعدّل صفًّا ولا حذفت الفواتير المكرّرة السبع من قاعدة التشغيل — الحذف على
  قاعدة ذات سلسلة تجزئة سبق أن أنتج تحذيرات `HashChain` في فحص سابق، فلا
  أفعله بلا تصريح.
- `0017/0018` طُبِّقتا على التطوير فقط. **أي بيئة أخرى تسحب هذا الفرع بلا
  `migrate` سترى الخطأ نفسه** — وهو بند يستحق سطرًا صريحًا في runbook النشر.
