...
### البوابة — تُلصَق في رأس كل كتلة، بلا استثناء

الخوادم الثلاثة تحمل نفس المستودع ونفس `docker-compose.yml`، وفيه خدمات
`web_live` و`db_live` و`celery_live` **على كل واحد منها**. فأمر نشر إنتاج ينفَّذ
على خادم التدريب لا يخطئ ولا يشتكي — يجد منظومة اسمها «live» ويعمل عليها.

حدث هذا يوم 2026‑08‑18: نُفِّذت مرحلة النشر كاملة على `srv1596748`. أوقفت حاويتين
هناك، وسقطت عند `django.setup()`، ولم تلمس الإنتاج. ما كان يُفترض أن يمنعها هو
سطر `echo "══ $(hostname) ══"` في رأس المرحلة السابقة — **وهو يُقرأ أو لا يُقرأ.**
`echo` ليس حارسًا. هذا هو الحارس:

```bash
IP=$(hostname -I | awk '{print $1}')
[ "$IP" = "72.62.239.220" ] || { echo "✗ الخادم الخطأ: $IP — توقّف"; exit 1; }
echo "✓ $(hostname) · $IP"
cd /root/tadgeeg   # أو مسار المستودع على الإنتاج
C="docker compose -f deployment/docker/docker-compose.yml"
```

في جلسة تفاعلية استبدل `exit 1` بـ`return 1` — أو نفّذ الكتلة عبر
`bash -c '...'` حتى يكون الخروج حقيقيًّا. **لا تُشغّل أي كتلة أدناه قبل أن تطبع `✓`.**
...
**التوقّف المقيس على الإنتاج، 2026‑08‑18:**

```
11:06:44  stop web_live celery_live
11:07:40  الهجرة انتهت              ← 46 ثانية · البروفة قالت 42
11:12:11  gunicorn Listening        ← ~دقيقتان إقلاع
```

**نحو ثلاث دقائق آليًّا.** الهجرة أصغر جزء فيها؛ الإقلاع هو الأطول — 419 ملفًّا
ثابتًا يُعاد جمعها، ثم بذر الخطط والإضافات والشركاء. انشر في نافذة منخفضة الحركة.

> التقدير الأول هنا كان «دقيقة ونصف»، مبنيًّا على زمن الهجرة وحده. الإقلاع كان
> ضعف ذلك التقدير كلّه.
