...
class SeedBillingPlansTests(TestCase):
    ...
    def test_creates_nine_plans(self):
        self.assertEqual(Plan.objects.count(), 9)
        ...
        self.assertEqual(codes, {
            ...
            PlanCode.ACCOUNTING_PARTNER.value,
            PlanCode.ACCOUNTING_PROFESSIONAL.value,
            PlanCode.ACCOUNTING_ENTERPRISE.value,
        })
    ...
    def test_starter_has_100_invoices_at_149(self):
        p = Plan.objects.get(code=PlanCode.STARTER)
        self.assertEqual(p.invoice_limit, 100)
        self.assertEqual(p.price, Decimal("149.00"))
        self.assertFalse(p.is_free)
        self.assertFalse(p.is_trial)

    def test_business_has_2000_invoices_at_599(self):
        p = Plan.objects.get(code=PlanCode.BUSINESS)
        self.assertEqual(p.invoice_limit, 2000)
        self.assertEqual(p.price, Decimal("599.00"))

    def test_professional_has_5000_invoices_at_999(self):
        p = Plan.objects.get(code=PlanCode.PROFESSIONAL)
        self.assertEqual(p.invoice_limit, 5000)
        self.assertEqual(p.price, Decimal("999.00"))
